<template>
  <div id='app' :class="getNodeEnv">
    <i class='exchange circular icon'
       @click='$store.commit("switchFlexDirection")'
       :style='{color: getColor, borderColor: getColor}'>
    </i>
    <icon name="color"
          v-bind:style="{color: getColor}"
          scale="4" id="colorpicker"
          @click.native="picker = !picker">
    </icon>
    <i class="erase circular icon" @click="$store.commit('setBaseColor')"
       :style='{color: getColor, borderColor: getColor}'></i>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...mapGetters(['getColor', 'getDarkerColor', 'getFlexDirection', 'getLinkColor', 'getNodeEnv']),
      color: {
        get () {
